...
if (confirm("Почати тестування?")) {
    ...
    //---
    function getSeasonByStartMonth(month) {
        if (month < 1 || month > 12)
            throwError('Некоректний номер місяця');
        const entry = seasonTuple.find(([m]) => m === month);
        if (!entry)
            throwError('Це не перший місяць пори року');
        return entry[1];
    }
    //---
    ...
    function getLastDigitOrSymbolOfNumber(input) {
        ...
        if (typeof input === 'string') {
            if (input.length === 0)
                return throwError('Рядок порожній');
    ...
    let SeasonEnum;
    (function (SeasonEnum) {
        SeasonEnum[SeasonEnum["Winter"] = 1] = "Winter";
        SeasonEnum[SeasonEnum["Spring"] = 2] = "Spring";
        SeasonEnum[SeasonEnum["Summer"] = 3] = "Summer";
        SeasonEnum[SeasonEnum["Fall"] = 4] = "Fall";
    ...
    //---
    let input = (_a = prompt(`Введіть номер місяця або назву місяця(En)`)) !== null && _a !== void 0 ? _a : '';
    ...
    //---
    function generateAndCountDays(size) {
        let weekendCount = 0;
        let holidayCount = 0;
        const days = [];
    ...
    //---
    function compareDaysCount(weekendCount, holidayCount) {
        if (weekendCount > holidayCount)
            return `Вихідних днів було більше`;
        if (holidayCount > weekendCount)
            return `Святкових днів днів було більше`;
    ...
    //---
    function getEnabledSettings(pairs) {
        const result = [];
    ...
    //---
    function getClosestPlace(userLocation, markers) {
        if (markers.length === 0)
            throw new Error("Маркерів немає");
    ...
    const userLocation = [50.00, 30.00];
